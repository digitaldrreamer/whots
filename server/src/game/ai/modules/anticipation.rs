use crate::game::{ai::types::{Candidate, ModuleContext}, types::SHAPES};
...
pub fn anticipation(candidate: &Candidate, ctx: &ModuleContext<'_>) -> f64 {
    ...
    let total_remaining: f64 = SHAPES.iter().map(|s| *ctx.shape_remaining.get(s).unwrap_or(&0) as f64).sum();
    ...
    let next_hand_size = ctx.opponent_hand_sizes.get(next_idx).copied().unwrap_or(5).max(0) as f64;
