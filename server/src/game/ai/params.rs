...
pub fn select_move_with_params(
    state: &GameState,
    seat_index: usize,
    ...
    let chosen =
        if params.bluff_rate > 0.0 && scored.len() > 1 && rng.gen::<f64>() < params.bluff_rate {
            scored[1].0
        } else {
            scored[0].0
        };
...
pub fn select_worst_move_with_params(
    state: &GameState,
    seat_index: usize,
    params: &DifficultyParams,
) -> Action {
